...
class QANetEncoderBlock(tf.keras.Model):
    ...
    def __init__(self, 
        ...
        super().__init__()
        self.conv_stack = Stack([QANetConvBlock(hidden_size, 7, dropout) for _ in range(n_conv)])
...
class QANet(tf.keras.Model):
    ...
    def __init__(self, 
        ...
        self.context_query_attention = ContextQueryAttention(None, self.d_model)
    ...
    def call(self, inputs, padding_mask=None, shift_target_sequence_right=True, training=True):
        context, question, context_characters, question_characters, answer_index1, answer_index2, qa_id = inputs
        ...
        context_query_mask = self._convert_padding_masks_to_context_query_mask(question_mask, context_mask)
        context_mask = self._convert_padding_mask_to_attention_mask(context, context_mask)
        question_mask = self._convert_padding_mask_to_attention_mask(question, question_mask)

        context_embedding = self.input_embedding((context, context_characters))
        question_embedding = self.input_embedding((question, question_characters))

        context_encoding = self.embedding_encoder(context_embedding, self_attention_mask=context_mask)
        question_encoding = self.embedding_encoder(question_embedding, self_attention_mask=question_mask)
        
        context_query_attention = self.context_query_attention((question_encoding, context_encoding),
                                                               mask=context_query_mask)
        context_query_projection = self.model_encoder_projection(context_query_attention)
        output = self.model_encoder(context_query_projection, self_attention_mask=context_mask)

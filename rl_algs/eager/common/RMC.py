...
import numpy as np
from rl_algs.eager.common.attention import MultiHeadAttentionMap
...
class RelationalMemoryCoreCell(tf.keras.Model):
    ...
    def __init__(self, mem_slots, head_size, num_heads=1,forget_bias=1.0 **kwargs):
        ...
        self.attention_layer = MultiHeadAttentionMap(self._num_heads, self._mem_size)
    ...
    def call(self, inputs, states):
        ...
        memory_proposal = self.g_phi(memory_proposal) + memory_proposal # Second residual block
        ...
        forget_gate = tf.keras.activations.sigmoid(self.u_f(h_state) + np.expand_dims(self.w_f(inputs),axis=1))
        # Generate the input gate. 
        input_gate = tf.keras.activations.sigmoid(self.u_i(h_state) + np.expand_dims(self.w_i(inputs),axis=1))
        # Generate the output gate.
        output_gate = tf.keras.activations.sigmoid(self.u_o(h_state) + np.expand_dims(self.w_o(inputs),axis=1))

        # Generate the updated memories.
        new_memories = tf.keras.activations.sigmoid(m_state * forget_gate + self._forget_bias) + memory_proposal * tf.keras.activations.sigmoid(input_gate)
        new_hidden = tf.keras.activations.tanh(m_state) * tf.keras.activations.sigmoid(output_gate)

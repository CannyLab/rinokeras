...
class MaskInput(Layer):
    ...
    def __init__(self, percentage: float, mask_token: int, n_symbols: Optional[int] = None, *args, **kwargs) -> None:
    ...
    def call(self, inputs: tf.Tensor, valid_mask: Optional[tf.Tensor] = None, n_symbols: Optional[int] = None):
        """
        Args:
        ...
        if discrete:
            ...
            masked_inputs += (K.random_normal(K.shape(masked_inputs)) + inputs) * random_bert_mask
